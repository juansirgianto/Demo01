...
var MarkerNormal = class extends AbstractStandardMarker {
  ...
  update(config) {
    ...
    if (this.config.image && !this.config.size) {
      throw new import_core13.PSVError(`missing marker ${this.id} size`);
    }
    if (this.config.size) {
      this.needsUpdateSize = false;
      this.state.size = this.config.size;
  ...
  update(config) {
    ...
    if (this.config.svgStyle) {
      Object.entries(this.config.svgStyle).forEach(([prop, value]) => {
        element.setAttributeNS(null, import_core14.utils.dasherize(prop), value);
      });
    ...
    try {
      ...
      if (this.isPolyline && actualPoly.length > 1) {
        throw new import_core14.PSVError(`polylines cannot have holes`);
      }
      if (this.isPixels) {
        this.definition = actualPoly.map((coords) => {
          return coords.map((coord) => {
            let sphericalCoord;
            if (import_core14.utils.isExtendedPosition(coord)) {
      ...
      throw new import_core14.PSVError(`invalid marker ${this.id} position`, e);
    }
...
var MarkerSvg = class extends AbstractStandardMarker {
  ...
  update(config) {
    ...
    if (this.config.svgStyle) {
      Object.entries(this.config.svgStyle).forEach(([prop, value]) => {
        svgElement.setAttributeNS(null, import_core15.utils.dasherize(prop), value);
      });
    } else {
...
// src/MarkersPlugin.ts
var getConfig = import_core16.utils.getConfigParser(
  {
...
function getMarkerCtor(config) {
  ...
  switch (type) {
    ...
    case "path":
      return MarkerSvg;
    default:
      throw new import_core16.PSVError("invalid marker type");
  }
}
var MarkersPlugin = class extends import_core16.AbstractConfigurablePlugin {
  ...
  init() {
    super.init();
    import_core16.utils.checkStylesheet(this.viewer.container, "markers-plugin");
    this.viewer.addEventListener(import_core16.events.ClickEvent.type, this);
    this.viewer.addEventListener(import_core16.events.DoubleClickEvent.type, this);
    this.viewer.addEventListener(import_core16.events.RenderEvent.type, this);
    this.viewer.addEventListener(import_core16.events.ConfigChangedEvent.type, this);
    this.viewer.addEventListener(import_core16.events.ObjectEnterEvent.type, this);
    this.viewer.addEventListener(import_core16.events.ObjectHoverEvent.type, this);
    this.viewer.addEventListener(import_core16.events.ObjectLeaveEvent.type, this);
    this.viewer.addEventListener(import_core16.events.ReadyEvent.type, this, { once: true });
  }
  /**
   * @internal
   */
  destroy() {
    ...
    this.viewer.removeEventListener(import_core16.events.ClickEvent.type, this);
    this.viewer.removeEventListener(import_core16.events.DoubleClickEvent.type, this);
    this.viewer.removeEventListener(import_core16.events.RenderEvent.type, this);
    this.viewer.removeEventListener(import_core16.events.ObjectEnterEvent.type, this);
    this.viewer.removeEventListener(import_core16.events.ObjectHoverEvent.type, this);
    this.viewer.removeEventListener(import_core16.events.ObjectLeaveEvent.type, this);
    this.viewer.removeEventListener(import_core16.events.ReadyEvent.type, this);
  ...
  handleEvent(e) {
    switch (e.type) {
      case import_core16.events.ReadyEvent.type:
        if (this.config.markers) {
          this.setMarkers(this.config.markers);
          delete this.config.markers;
        }
        break;
      case import_core16.events.RenderEvent.type:
        this.renderMarkers();
        break;
      case import_core16.events.ClickEvent.type:
        this.__onClick(e, false);
        break;
      case import_core16.events.DoubleClickEvent.type:
        this.__onClick(e, true);
        break;
      case import_core16.events.ObjectEnterEvent.type:
      case import_core16.events.ObjectLeaveEvent.type:
      case import_core16.events.ObjectHoverEvent.type:
        ...
        break;
      case "mouseenter": {
        const marker = this.__getTargetMarker(import_core16.utils.getEventTarget(e));
      ...
      case "mousemove": {
        const marker = this.__getTargetMarker(import_core16.utils.getEventTarget(e), true);
  ...
  addMarker(config, render = true) {
    if (this.markers[config.id]) {
      throw new import_core16.PSVError(`marker "${config.id}" already exists`);
    }
  ...
  getMarker(markerId) {
    ...
    if (!this.markers[id]) {
      throw new import_core16.PSVError(`cannot find marker "${id}"`);
    }
  ...
  gotoMarker(markerId, speed = this.config.gotoMarkerSpeed) {
    ...
    if (!speed) {
      ...
      if (!import_core16.utils.isNil(marker.config.zoomLvl)) {
  ...
  toggleMarker(markerId, visible) {
    const marker = this.getMarker(markerId);
    marker.config.visible = import_core16.utils.isNil(visible) ? !marker.config.visible : visible;
    this.renderMarkers();
  }
  /**
  ...
  showMarkersList() {
    ...
    this.viewer.panel.show({
      ...
      noMargin: true,
      clickHandler: (target) => {
        const li = import_core16.utils.getClosest(target, ".psv-panel-menu-item");
  ...
  renderMarkers() {
    ...
    Object.values(this.markers).forEach((marker) => {
      ...
      if (marker.domElement) {
        import_core16.utils.toggleClass(marker.domElement, "psv-marker--visible", isVisible);
      }
      if (!isVisible) {
        marker.hideTooltip();
  ...
  __getTargetMarker(target, closest = false) {
    if (target instanceof Node) {
      const target2 = closest ? import_core16.utils.getClosest(target, ".psv-marker") : target;
...
MarkersPlugin.VERSION = "5.13.3";
MarkersPlugin.configParser = getConfig;
MarkersPlugin.readonlyOptions = ["markers"];

// src/index.ts
import_core17.DEFAULTS.lang[MarkersButton.id] = "Markers";
import_core17.DEFAULTS.lang[MarkersListButton.id] = "Markers list";
(0, import_core17.registerButton)(MarkersButton, "caption:left");
(0, import_core17.registerButton)(MarkersListButton, "caption:left");
